...
function scoreColor(sc) {
  if (sc >= 80) return { bg: '#1a4d1a', text: '#86e986' }
  if (sc >= 65) return { bg: '#2d5a1b', text: '#a3ed6b' }
  if (sc >= 50) return { bg: '#5a4a00', text: '#f0d060' }
  if (sc >= 35) return { bg: '#5a2800', text: '#ff9966' }
  return              { bg: '#2a0000', text: '#ff6666' }
}
function Arrow({ deg, size = 14, opacity = 1 }) {
  return <span style={{ display: 'inline-block', transform: `rotate(${deg}deg)`, fontSize: size, lineHeight: 1, opacity }}>↑</span>
}
              ...
              <table style={s.table}>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    {allCols.map((h, i) => <td key={i} style={{ ...s.cell, color: '#94a3b8' }}><Arrow deg={h.wind_dir} /></td>)}
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    {allCols.map((h, i) => { const sc = h.scores?.[sport] ?? 0; const c = scoreColor(sc); return <td key={i} style={{ ...s.cell, background: c.bg, color: c.text, fontWeight: 700 }}>{Math.round(sc)}</td> })}
                  </tr>
                </tbody>
              </table>
            </div>
